...
### Application Flow (Public-First Approach)

```
EventManagementApp.java (Main Entry Point)
    └── PublicEventBrowserController (Default Landing Page - Anonymous Access)
        ├── "Sign In" Button → Login Screen (for authenticated users)
        └── "Register" Button → Registration Screen (create new account)
...
### Entry Point
**File**: `src/main/java/group_3/ui/EventManagementApp.java`
- Launches with **Public Event Browser** as default landing page
- Anonymous visitors can browse events without logging in
- Users click "Sign In" or "Register" to access authenticated features
...
Anonymous visitors land directly on the **Public Event Browser** when they launch the app - **no login required**.
...
mvn clean javafx:run -Djavafx.mainClass=group_3.ui.AuthApp
```

### Option 2: Direct Event Management (No Auth)
```bash
# Run Event Management directly
mvn clean javafx:run
```

### Option 3: IDE Run
Right-click on `AuthApp.java` → Run
