...
public class EventStatisticsServiceImpl implements EventStatisticsService {
    ...
    public List<EventStatistics> getAllEventStatistics() {
        ...
        for (Event event : events) {
            try {
                ...
                // Skip events with non-numeric IDs
                System.err.println("Warning: Skipping event with non-numeric ID: " + event.getEventId());
            }
        }
    ...
    public Optional<SessionStatistics> getSessionStatistics(int sessionId) {
        ...
        int eventId = session.getEventId() != null ? Integer.parseInt(session.getEventId()) : 0;
    ...
    public List<SessionStatistics> getEventSessionStatistics(int eventId) {
        ...
        for (Session session : sessions) {
            try {
                int sessionId = Integer.parseInt(session.getSessionId());
                Optional<SessionStatistics> stats = getSessionStatistics(sessionId);
                stats.ifPresent(statsList::add);
            } catch (NumberFormatException e) {
                // Skip sessions with non-numeric IDs
                System.err.println("Warning: Skipping session with non-numeric ID: " + session.getSessionId());
            }
        }
